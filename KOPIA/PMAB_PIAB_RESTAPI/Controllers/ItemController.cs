...
namespace PMAB_PIAB_RESTAPI.Controllers
{
    public class ItemController : Controller
    {
        ...
        {
            DatabaseContext database = new();
            ItemCreateEditVM item = await database.Items
                .Where(item => item.Id == id)
                .Select(item => new ItemCreateEditVM()
                {
                    Id = item.Id,
                    Name = item.Name,
                    Description = item.Description,
        ...
        public async Task<IActionResult> Edit(int id, [FromBody] ItemCreateEditVM itemCreateEditVM)
        {
            ...
            item.Description= itemCreateEditVM.Description;
            item.CategoryId = itemCreateEditVM.CategoryId;
            item.Price = itemCreateEditVM.Price;
            item.ImageUrl = itemCreateEditVM.ImageUrl;
